...
@time if (@isdefined reload_files) ? reload_files : true
    ...
    px = Lv * HDF5.h5read("f_reduced.h5", "px")[:, :, 1:Nt]
    py = Lv * HDF5.h5read("f_reduced.h5", "py")[:, :, 1:Nt]
...
anim = @animate for i in timeran
    ...
    p3 = heatmap( box.x[1], box.x[2], px[:,:, i]',
                  title = "\$ p_x \$",
                  xlabel = "\$ x \\lambda_{D}^{-1} \$",
                  ylabel = "\$ y \\lambda_{D}^{-1} \$",
                  cbar = :bottom,
    ...
    p4 = heatmap( box.x[1], box.x[2], py[:,:, i]',
                  title = "\$ p_y \$",
                  xlabel = "\$ x \\lambda_{D}^{-1} \$",
                  ylabel = "\$ y \\lambda_{D}^{-1} \$",
                  cbar = :bottom,
    ...
    plot(p1, p2, p3, p4,
         titlefontsize = 10,
         dpi = 300,
         layout = @layout [ a{0.2h} ; b ; c d ]
         )

    next!(pbar)
end

mp4(anim, "$dirname/momentum.mp4", fps = 24)
